...
MODEL_NAME = 'lgbm_energy_model' # Changed model name
...
# --- 3. Model Training ---
print("Initializing and training the LightGBM model for total energy consumption...")

# Define the model with hyperparameters
# LightGBMModel automatically handles feature engineering like lags based on input_chunk_length
input_chunk_length = 24 * 7
output_chunk_length = 24

model_energy = LightGBMModel(
    lags=input_chunk_length, # Use lags up to input_chunk_length
    lags_future_covariates=[0, output_chunk_length-1], # Use future covariates for the prediction horizon
    output_chunk_length=output_chunk_length,
    random_state=42,
    model_name=MODEL_NAME,
    work_dir=MODEL_OUTPUT_DIR,
    # Removed save_checkpoints=True as we will manually save with joblib
    force_reset=True,
)
...
model_energy.fit(
    ...
    # Removed verbose=True as LightGBMModel does not support it
)
...
model_save_path = os.path.join(model_save_dir, 'model.joblib')
joblib.dump(model_energy, model_save_path)
