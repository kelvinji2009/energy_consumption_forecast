...
function ModelTraining() {
  ...
  // 获取模型参数配置
  const getModelParameters = (modelType) => {
    const baseParams = [
      {
    ...
    return [...baseParams, ...(modelSpecificParams[modelType] || [])];
  };

  // 初始化参数
  useEffect(() => {
    const modelParams = getModelParameters(modelType);
    const initialParams = {};
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      try {
        ...
        setError('Failed to fetch assets. Please check your API key.');
      }
    };
    fetchAssets();
  }, []);
  ...
  const handleTrain = async () => {
    ...
    if (dataInputMethod === 'upload') {
      ...
      const parametersArray = Object.entries(parameters).map(([name, value]) => {
        const paramConfig = getModelParameters(modelType).find(p => p.name === name);
        return {
          parameter_name: name,
          parameter_value: value.toString(),
          parameter_type: paramConfig?.type || 'str',
          parameter_category: paramConfig?.category || 'model'
        };
      });
              ...
              <div key={param.name} style={{ marginBottom: '1rem' }}>
                ...
                {param.type === 'int' ? (
                  <input
                    ...
                    onChange={e => updateParameter(param.name, parseInt(e.target.value, 10))}
                    min={param.min}
                    max={param.max}
                    disabled={loading}
                    style={{
                ...
                <small style={{
                  ...
                  {param.category && (
                    <span style={{
                      ...
                      background: param.category === 'model' ? '#e6fffa' : 
                                 param.category === 'training' ? '#fff5f5' : '#f0fff4',
                      color: param.category === 'model' ? '#234e52' : 
                             param.category === 'training' ? '#742a2a' : '#22543d',
                      borderRadius: '4px',
                      fontSize: '0.7rem'
                    }}>
                      {param.category}
